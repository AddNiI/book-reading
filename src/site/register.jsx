    ...
    return (
        <div>
            ...
            <main style={{display: isPhone ? 'block' : (isPad ? 'block': 'flex')}}>
                <div style={{backgroundImage: isPhone ? `url(${background_picture_phone})` : (isPad ? `url(${background_picture_pad})` : `url(${background_picture_desctop})`), backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', aspectRatio: isPhone ? '320 / 623' : (isPad ? '384 / 365' : '113 / 178'), height: isPhone ? 'auto' : (isPad ? 'auto' : 'calc(100vh - 60px)'), width: isPhone ? '100%' : (isPad ? '100%' : 'auto') }}>
                    <div style={{ backgroundColor: '#091E3FCC', width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <div id='registration-form' style={{ backgroundColor: isPhone ? '' : '#fff', height: isPhone ? '539px' : '610px', width: isPhone ? '270px' :'360px', paddingLeft: isPhone ? '0' : '40px'}}>
                            <button 
                                onClick={() => googleReg()} 
                                style={{cursor: 'pointer', fontFamily: '"Roboto", serif', fontWeight: 700, color: '#707375', border: '0', backgroundColor: '#F5F7FA', padding: '11px 49px 11px 14px', display: 'flex', margin: isPhone ? '0 0 0 60px' : '39px 0 0 85px', boxShadow: '0 2px 2px #091E3F26'}}
                            >
                                <img src={google_logo} alt='G' style={{width: '18px', padding: '0 17px 0 0'}}></img>
                                <p style={{margin: '0'}}>Google</p>
                            </button>
                            <div style={{marginTop: isPhone ? '16px' : '22px'}}>
                                <p style={{fontFamily: '"Montserrat", serif', fontWeight: 500, display: 'flex', margin: '0 0 11px', color: isPhone ? '#fff' : '#898F9F'}}>Ім’я<span style={{margin: '0 0 0 5px', color: '#f00'}}>*</span></p>
                                <input type='text' name='name' value={register.name} onChange={onChange} placeholder='...' style={{fontFamily: '"Montserrat", serif', fontWeight: 400, color: '#A6ABB9', backgroundColor: '#F5F7FA', border: '0', padding: '0 0 0 13px', boxShadow: 'inset 0 1px 2px #1D1D1B26', width: isPhone ? '257px' : '307px', height: '42px'}} />
                            </div>
                            <div style={{marginTop: isPhone ? '16px' : '18px'}}>
                                ...
                                <input type='email' name='email' value={register.email} onChange={onChange} placeholder='your@email.com' style={{fontFamily: '"Montserrat", serif', fontWeight: 400, color: '#A6ABB9', backgroundColor: '#F5F7FA', border: '0', padding: '0 0 0 13px', boxShadow: 'inset 0 1px 2px #1D1D1B26', width: isPhone ? '257px' : '307px', height: '42px'}} />
                            </div>
                            <div style={{marginTop: isPhone ? '16px' : '18px'}}>
                                ...
                                <input type='password' name='password' value={register.password} onChange={onChange} placeholder='...' style={{fontFamily: '"Montserrat", serif', fontWeight: 400, color: '#A6ABB9', backgroundColor: '#F5F7FA', border: '0', padding: '0 0 0 13px', boxShadow: 'inset 0 1px 2px #1D1D1B26', width: isPhone ? '257px' : '307px', height: '42px'}} />
                            </div>
                            <div style={{marginTop: isPhone ? '16px' : '18px'}}>
                                <p style={{fontFamily: '"Montserrat", serif', fontWeight: 500, display: 'flex', margin: '0 0 11px', color: isPhone ? '#fff' : '#898F9F'}}>Підтвердити пароль<span style={{margin: '0 0 0 5px', color: '#f00'}}>*</span></p>
                                <input type='password' name='passwordRepeat' value={register.passwordRepeat} onChange={onChange} placeholder='...' style={{fontFamily: '"Montserrat", serif', fontWeight: 400, color: '#A6ABB9', backgroundColor: '#F5F7FA', border: '0', padding: '0 0 0 13px', boxShadow: 'inset 0 1px 2px #1D1D1B26', width: isPhone ? '257px' : '307px', height: '42px'}} />
                            </div>
                            <button onClick={submitData} style={{cursor: 'pointer', marginTop: isPhone ? '30px' : '36px', color:'#fff', border: '0', backgroundColor: '#FF6B08', padding: isPhone ? '20px 64px 21px' : '20px 89px 21px', display: 'flex', justifyContent: 'center'}}><h3 style={{fontFamily: '"Montserrat", serif', fontWeight: 600, margin: '0'}}>Зареєструватися</h3></button>
                            <Link to="/login" style={{textDecoration: 'none'}}>
                                <h5 style={{fontFamily: '"Montserrat", serif', fontWeight: 500, margin:  isPhone ? '19px 0 0 67px' :'19px 0 0 92px', color:  isPhone ? '#fff' : '#898F9F'}}>Вже з нами? <span style={{textDecoration: 'underline', color: '#FF6B08'}}>Увiйти</span></h5>
                            </Link>
                            <div style={{display: isPhone ? 'block' : 'flex'}}>
                                <Link to="/terms-of-service" style={{ textDecoration: 'none'}}>
                                    <p style={{fontFamily: '"Montserrat", serif', fontWeight: 500, fontSize: isPhone ? '14px' : '12px',color: '#FF6B08', margin: '15px 0'}}>Умови використання</p>
                                </Link>
                                <Link to="/privacy-policy" style={{ textDecoration: 'none'}}>
                                    <p style={{fontFamily: '"Montserrat", serif', fontWeight: 500, fontSize: isPhone ? '14px' : '12px', color: '#FF6B08', margin: isPhone ? '15px 0' : '15px 0 0 15px'}}>Політика конфіденційності</p>
                                </Link>
                            </div>
                        </div>
                    </div>
                ...
                {isPhone ? (
                        ...
                        <div style={{flex: '1',display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'inset 0 2px 2px #091E3F1A', flexDirection: 'column'}}>
                            <p style={{margin: isPad ? '60px 0 0 0' : '0', fontFamily: '"Abril Fatface", serif', fontWeight: 400, color: '#242A37', fontSize: '34px'}}>Books Reading</p>
                            <div>
