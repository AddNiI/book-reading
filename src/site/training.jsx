...
import { PagesState } from './pagesstate.jsx';
...
function Training() {
    ...
    const { currentUser, setCurrentUser, setUsers } = useContext(LoginsState);
    ...
    const addDate = () => {
        ...
        if (data4 !== '' && countPages !== '') {
            const newPage = {date: data4, time: nowTime, pages: countPages, userid: uid, whatuserid: uid};
            setPages(prev => [...prev, newPage]);
            setCurrentUserPages(prev => [...prev, newPage]);
            setData4('');
            setCountPages('');
        }
    }
        ...
        return (
            <div onClick={onClose} style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center'}} >
                <div onClick={e => e.stopPropagation()} style={{background:'#fff', width: '570px', height: '408px', padding: '20px', borderRadius: '6px '}}>
                    <p>Якщо Ви вийдете з програми<br />незбережені дані будуть втрачені</p>
                    <button onClick={onClose}>Вiдмiна</button>
                    <Link to="/login">
                        <button>Вийти</button>
                    </Link>
                </div>
        ...
        <div>
            ...
            <main style={{backgroundColor: '#F6F7FB', display: 'flex', padding:'0 0 0 calc(50% - 601px)'}}>
                ...
                <div>
                    ...
                    <div style={{display: 'flex'}}>
                        <div>
                            ...
                            <div style={{display: 'flex'}}>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 10px 0 0'}}>{String(yearCountdown.days).padStart(2, '0')}</p>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 10px 0 0'}}>{String(yearCountdown.hours).padStart(2, '0')}</p>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 10px 0 0'}}>{String(yearCountdown.minutes).padStart(2, '0')}</p>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 0 0 0'}}>{String(yearCountdown.seconds).padStart(2, '0')}</p>
                            </div>
                            <div style={{display: 'flex'}}>
                                <p style={{fontSize: '12px', color: '#898F9F', margin: '5px 28px 0 0'}}>ДНІ</p>
                                <p style={{fontSize: '12px', color: '#898F9F', margin: '5px 22px 0 0'}}>ГОД</p>
                                <p style={{fontSize: '12px', color: '#898F9F', margin: '5px 10px 0 0'}}>ХВ</p>
                                <p style={{fontSize: '12px', color: '#898F9F', margin: '5px 0 0 0'}}>СЕК</p>
                            </div>
                        </div>
                        <div>
                            <p>До досягнення мети залишилось</p>
                            <div style={{display: 'flex'}}>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 10px 0 0'}}>{String(goalCountdown.days).padStart(2, '0')}</p>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 10px 0 0'}}>{String(goalCountdown.hours).padStart(2, '0')}</p>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 10px 0 0'}}>{String(goalCountdown.minutes).padStart(2, '0')}</p>
                                <p style={{fontWeight: 700, fontSize: '24px', margin: '10px 0 0 0'}}>{String(goalCountdown.seconds).padStart(2, '0')}</p>
                            </div>
                            <div style={{display: 'flex'}}>
                                <p style={{fontSize: '12px', color: '#898F9F', margin: '5px 28px 0 0'}}>ДНІ</p>
                                <p style={{fontSize: '12px', color: '#898F9F', margin: '5px 22px 0 0'}}>ГОД</p>
                    ...
                    <div>
                        ...
                        {reading.length === 0 && finishing.length === 0 ? (
                            ...
                            <>
                                {finishing.map(book => (
                                    ...
                                    <input type='checkbox' onChange={() => unfinishBook(book.id)} checked='true' style={{cursor: 'pointer', appearance: 'none', width: '15px', height: '15px', border: '1px solid #FF6B08', borderRadius: '0', backgroundColor: '#FF6B08', margin: '24px 24px 0 0'}}  />
                        ...
                        {wantread.length === 0 ? (
                            ...
                            <button onClick={startTraining} style={{color: '#fff', backgroundColor: '#FF6B08', border: '0', fontFamily: '"Montserrat", serif', fontWeight: 600, fontSize: '16px', padding: '12px 30px', margin: '40px 0 40px calc(50% - 100px)'}}>Почати тренування</button>
                        )}
                        <div style={{width: '886px', height: '340px', backgroundColor: '#fff', margin: training ? '40px 0 36px calc(50% - 443px)': '0 0 36px calc(50% - 443px)'}}>
                ...
                <div>
                    ...
                    <div style={{fontFamily: '"Montserrat", serif', fontWeight: 500, display: 'flex', backgroundColor: '#fff', margin: '0 0 40px 41px'}}>
                        <div>
                            <p style={{fontFamily: '"Open Sans", serif', fontWeight: 700, fontSize: '45px', color: '#091E3F', margin: '0', width: training ? '66px' : '100px', height: training ? '66px' : '100px', margin: training ? '72px 22px 0 17px' : '55px 20px 14px 28px', backgroundColor: '#F5F7FA'}}>{training ? (reading.length + finishing.length) : selectBook.length}</p>
                            <p style={{fontSize:'12px',color: '#898F9F', margin: training ? "14px 0 76px 22px" : '14px 20px 55px 45px', width: '66px'}}>Кiлькiсть книжок</p>
                        </div>
                        <div>
                            <p style={{fontFamily: '"Open Sans", serif', fontWeight: 700, fontSize: '45px', color: '#091E3F', margin: '0', width: training ? '66px' : '100px', height: training ? '66px' : '100px', margin: training ? '72px 22px 0 0' : '55px 0 14px 0', backgroundColor: '#F5F7FA'}}>{data3}</p>
                            <p style={{fontSize:'12px',color: '#898F9F', margin: training ? "14px 0 76px 0" : '14px 0 55px 0', width: '66px'}}>Кількість днів</p>
                        </div>
                        {training ? (
                            <div>
                                <p style={{fontFamily: '"Open Sans", serif', fontWeight: 700, fontSize: '45px', color: '#FF6B08', margin: '0', width: '66px', height: '66px', margin: '72px 0 0 0', backgroundColor: '#F5F7FA'}}>{reading.length}</p>
                    ...
                    {training ? (
                        <div style={{width: '240px', backgroundColor: '#fff', fontFamily: '"Montserrat", serif', fontWeight: 600, color: '#242A37', padding: '24px 16px 0 19px', marginLeft: '41px'}}>
                            ...
                            <button onClick={addDate} style={{color: '#fff', backgroundColor: '#FF6B08', border: '0', fontSize: '16px', padding: '11px 54px', margin: '20px 0 40px 0'}}>Додати результат</button>
                            <div style={{display: 'flex'}}>
